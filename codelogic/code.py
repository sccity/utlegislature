...
class UtahCode:
    ...
    def create_txt_file(self, title_number, title_data):
        ...
        with open(
            f"utcode/uca_title_{title_number}.txt", "w", encoding="utf-8"
        ) as file:
            ...
            for chapter in chapters:
                ...
                for section in sections:
                    section_catchline_text = (
                        section.find(".//catchline").text
                        if section.find(".//catchline") is not None
                        else ""
                    )
                    section_content = (
                        section.find(".//tab").tail.strip()
                        if section.find(".//tab") is not None
                        and section.find(".//tab").tail
                        else ""
                    )
                    ...
                    for subsection in subsections:
                        ...
                        xref_text = "; ".join(
                            xref.text.strip() if xref.text else "" for xref in xrefs
                        )
                        subsection_content = (
                            f"{xref_text} {subsection.text.strip()}"
                            if xref_text and subsection.text
                            else (xref_text or subsection.text or "").strip()
                        )
                        subsection_title = (
                            f"Utah Code Annotated § {subsection.attrib['number']}"
                        )

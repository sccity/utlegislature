...
class Status:
    def __init__(self, bill_year, bill_number, db_host, db_user, db_password, db_name):
        ...
        logging.debug(f"Initializing Status class for bill {self.bill_number} for year {self.bill_year}.")
    ...
    def get_status_entries(self, html_content):
        ...
        for row in entry_parser.status_entries:
            ...
            action = self.strip_tags(row[1]).strip().upper()  # Ensure action is uppercase
    ...
    def save_to_db(self, entry):
        try:
            ...
            """
            self.cursor.execute(select_query, (self.bill_number, self.bill_year, entry['date']))
            existing_record = self.cursor.fetchone()

            if existing_record:
                logging.debug(f"Duplicate entry found for {self.bill_number} on {entry['date']}, skipping insert.")
            else:
                guid = str(uuid.uuid4())
                insert_query = """
                ...
                self.cursor.execute(insert_query, (
                    guid, self.bill_number, self.bill_year, entry['date'], entry['action'], entry['location']
                ))
                self.connection.commit()
                logging.debug(f"Inserted record for {self.bill_number} on {entry['date']}.")
    ...
    def get_status(year):
        try:
            ...
            for bill in bills:
                ...
                scraper = Status(year, bill_number,
            ...
            logging.debug(f"Closed database connection after processing all bills for year {year}.")
